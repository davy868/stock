...
#本文件功能是将一个目录下的所有csv文件分别写入一个数据库中，一个csv文件一个数据表
...
file=get_file_name('/root/PycharmProjects/stock_pairs/data/')
...
engine = create_engine('mysql+pymysql://root:Luo0046!sq@localhost/stock_5min_from20151201?charset=utf8')





columns=['index','data','open','close','high','low','volume','code']

for i in range(len(file)):
    data=pd.read_csv(file[i],header=None,names=columns)
    data.to_sql(file[i][-10:-4],engine)
    print file[i][-10:-4]
    print i
